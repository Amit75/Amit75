...
PACKAGE="${OUTPUT_DIR}/aarulya-store-${SHA}-${STAMP}.tar.gz"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
printf 'AARULYA_STORE_HANDOFF_SOURCE_SHA=%s\n' "$SHA"
printf 'AARULYA_STORE_HANDOFF_PACKAGE=%s\n' "$PACKAGE"
printf 'AARULYA_STORE_PRODUCTION_DEPLOYED=false\n'
